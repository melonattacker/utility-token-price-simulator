...
def generate_brown_motion(previous_value: float, mu: float, sigma: float, dt: float, random_value: float):
    return previous_value * math.exp((mu - (sigma ** 2) / 2) * dt + sigma * math.sqrt(dt) * random_value)

def generate_ornstein_uhlenbeck_process(previous_value: float, mu: float, sigma: float, dt: float, random_value: float):
    theta: float = sigma / math.sqrt(2 * mu)
